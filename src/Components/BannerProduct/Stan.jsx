...
function Stan({data}) {
 ...
 <li className={`card ${style.product}  border-0 pb-3 mt-3 mb-5 col-3`}>
	<div className="">
        <Link to={`/stan/`} title="flex" className={style.cardimg}>
			...
			<div className="cardheader text-center">				
				<div className="">
					...
					<Link to="" className='text-dark '>منتجات الطباعة</Link>
						...
						<div className="mb-3">
					<Link to={`/stan/`} aria-label="flex" className="text-dark fs-4 fw-bold ">ستان</Link>

						</div>
<Link to={`/stan/`} className={` text-light border-0 ${style.cardbtn} rounded border border-danger mt-3 w-100  btn main-btn width-fluid text-center align-items-center `}   aria-describedby="هناك العديد من الأشكال المختلفة لهذا المنتج. يمكن اختيار الخيارات على صفحة المنتج">حدد ابعاد المنتج
</Link>
</div>
</li>
