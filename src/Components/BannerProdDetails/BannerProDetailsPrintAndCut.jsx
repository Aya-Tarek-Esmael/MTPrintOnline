...
function BannerProDetailsPrintAndCut() {
    const { id } = useParams();
    ...
    async function getProDetails() {
        let { data } = await axios.get(`https://mtb3a.arabiangeeks.net/api/products/18/details`);
        console.log(data);
        setProDetails(data);
        setTypesAndPrices({
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                {isPriceVisible && selectedType && (
                                    <>
                                        <div className='d-flex justify-content-between py-1'>
                                            <div className='col-6'>
                                                <div className="text-danger">x <span>{quantity}</span> <span className='fw-bold'>برنت اند كات</span></div>
                                            </div>
                                            <div>
                                                <span className='col-6 mx-3 text-danger fw-bold'>{(parseFloat(price * quantity)).toFixed(2)} EGP</span>
                                            </div>
                                        </div>
                                        {textareaValue.trim() !== '' || selectedFile !== null ? (
                                            <div className='col-12 '>
                                ...
                                <div className="d-flex w-100 mt-4">
                                    <div className="quantity col-4 mt-2">
                                        <label className="screen-reader-text d-none" htmlFor="quantity">كمية برنت اند كات</label>
                                        <input
                                            type="number"
                                            id="quantity"
...
export default BannerProDetailsPrintAndCut;
